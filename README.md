...
src/
  ...
  infrastructure/
    geolocation/
      browser-location-provider.ts # Geolocation watchPosition
    storage/
      dexie-railway-database.ts # Dexie.js IndexedDB & Bundled JSON投入
    even-g2/
      even-g2-adapter.ts       # Even G2 Bridge / Mock Adapter
      hud-renderer.ts          # HUDテキスト描画フォーマッタ
    logging/
      logger.ts                # 推定ログ配信
  data/sample/                 # 同梱サンプル鉄道データ (小田急線一部)
    lines.json
    stations.json
    track-segments.json
    metadata.json
...
npm run dev
```
ブラウザで `http://localhost:5173` を開くと、**Even G2 HUD プレビュー** および **デバッグパネル** が表示されます。
...
スマートフォン画面（またはブラウザ）上で「**GPSデモ再生**」ボタンを押すと、小田急小田原線のサンプル走行動作を模擬再生し、HUDおよびデバッグパネルの動作を確認できます。
